// Script to export Hardhat default accounts
const fs = require('fs');
...
let output = "═══════════════════════════════════════════════════════════════\n";
output += "           HARDHAT LOCAL NODE - DEFAULT ACCOUNTS\n";
output += "═══════════════════════════════════════════════════════════════\n\n";
output += "⚠️  WARNING: These accounts and private keys are publicly known.\n";
output += "    NEVER use them on mainnet or with real funds!\n";
...
accounts.forEach((account, index) => {
  ...
  output += `Private Key: ${account.privateKey}\n`;
  output += `Balance:     10000 ETH\n\n`;
});

output += "═══════════════════════════════════════════════════════════════\n";
output += "RECOMMENDED ACCOUNT FOR METAMASK (Account #0):\n";
output += "═══════════════════════════════════════════════════════════════\n\n";
output += `Address:     ${accounts[0].address}\n`;
output += `Private Key: ${accounts[0].privateKey}\n\n`;
output += "To import to MetaMask:\n";
output += "1. Click on account icon\n";
output += "2. Select 'Import Account'\n";
output += "3. Paste the private key above\n";
output += "4. Click 'Import'\n\n";

const filePath = path.join(__dirname, '..', 'hardhat-accounts.txt');
fs.writeFileSync(filePath, output);

console.log('✅ Accounts exported to hardhat-accounts.txt');
